...
# Objetivo: Definir diretório-base e todas as pastas-alvo da instalação.
...
# Objetivo: Declarar o que é obrigatório e o que é opcional copiar.
...
# Objetivo: Configurar logging no mesmo padrão do utils (arquivo + console).
...
# Objetivo: Copiar arquivo ou pasta, registrando sucesso/falha e removendo a origem.
...
# Objetivo: Criar a estrutura mínima do projeto (SEM criar _build).
def _criar_estrutura_minima():
    logging.info("Criando pastas do projeto…")
    for p in [
        #DEST_EXECUTORES,
        DEST_INSTALADORES, DEST_LOGS,
        DEST_CUMMINS, DEST_TRANSICOES, DEST_INMET, DEST_SIS_ELETRICO
    ]:
        p.mkdir(parents=True, exist_ok=True)
        logging.info(f"Pasta pronta: {p}")

# === [Seção inst-060: Execução principal] ==================================
# Objetivo: Validar essenciais, copiar opcionais e concluir instalação.
def main():
    _init_logger()
    ...
    # 5) Opcionais → _executores (desabilitado por decisão sua)
    ...
    # Opcional: tentar iniciar automaticamente (mantido comentado)
    #try:
